...
export function ListItemDemo() {
  return (
    ...
    >
      <DemoBlock label="notifi-link — with Learn More" code={`// notifi-link: icon + title + description + count badge + text link
<ListItem
  type="notifi-link"
  ...
  showDivider
  linkText="Learn more"
  onLinkClick={() => navigate('/invoices/4821')}
/>`}>
      ...
      <DemoBlock label="notifi-default — icon + count" code={`// notifi-default: icon + title + description + numeric count badge
<ListItem
  type="notifi-default"
  title="New Messages"
...
/>`}>
      ...
      <DemoBlock label="notifi-list — dot indicator + label" code={`// notifi-list: coloured dot + title + description + pill label
<ListItem
  type="notifi-list"
  title="Pending Approval"
  description="3 items need your sign-off."
  label="HIGH PRIORITY"
  showDivider
/>`}>
      ...
      <DemoBlock label="bill-list — inline icon + count + label" code={`// bill-list: doc icon + title + due date + amount string + category label
<ListItem
  type="bill-list"
...
/>`}>
      ...
      <DemoBlock label="bank-list — currency badge" code={`// bank-list: currency badge (GBP/USD/EUR) + account name + masked number
<ListItem
  type="bank-list"
...
/>`}>
      ...
      <DemoBlock label="minimal — count + chevron" code={`// minimal: title + numeric count + right chevron — clickable row
<ListItem
  type="minimal"
