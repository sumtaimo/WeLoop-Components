...
/**
 * "loading"  — blue (#1D32FF) — active upload / in-progress
 * "fail"     — red  (#EF4444) — error state
 * "complete" — green (#16A34A) — successfully finished
 */
export type ProgressBarVariant = "loading" | "fail" | "complete";

/** "sm" = 2px track · "md" = 8px track */
export type ProgressBarSize = "sm" | "md";

export interface ProgressBarProps {
  /** Progress 0–100 (will be clamped) */
  value?:          number;
  /** Denominator label for progress text (e.g. 200 for "200 MB") */
  max?:            number;
  ...
  required?:       boolean;
  /** Override progress text (e.g. "108 KB of 1.2 MB"). Auto-generated when `max` is set. */
  progressText?:   string;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6, width: "100%", ...style }}>
      ...
      {label && (
        <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
          ...
          {required && (
            <span style={{ color: "#EF4444", fontSize: 13, fontWeight: 500 }}>*</span>
          )}
        </div>
      )}
